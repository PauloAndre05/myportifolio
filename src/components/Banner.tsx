...
export function Banner () {
    return(
        <main className="h-[100vh] flex p-20 pt-44 justify-between items-center relative max-sm:p-0">
            <img 
                src={imgBanner} 
                className="absolute inset-0 w-full h-full bg-no-repeat object-cover opacity-5 max-sm:opacity-10" 
                alt="Imagem de Fundo"
            />
            <div className="flex flex-col gap-4 z-10 max-sm:min-w-[99%] max-sm:m-auto max-sm:border max-sm:text-center">
                ...
                <button className='w-40 p-2 rounded-full bg-four font-bold hover:opacity-50 max-sm:m-auto'>Download CV</button>
            </div>
